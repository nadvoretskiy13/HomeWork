...
public class User {
    ...
    public User(String id,
                ...
                Integer age,
                Boolean isWorker) {
        this(id, LocalDateTime.now(), login, password, confirmPassword,
                lastName, firstName, patronymic, age, isWorker);
    }
...
// Валидации
        ...
        if (age != null && age < 0) throw new
                ValidationException("Возраст не может быть отрицательным");
        this.id = id;
...
// Разрешим UUID с дефисами и набор из букв/цифр согласно примерувходных данных
    ...
    private void validatePassword(String pass, String confirm) {
        ...
        if (pass.matches("[A-Za-z]+"))
            throw new
                    ValidationException("Пароль не может состоять только из букв");
    ...
    private void validateName(String name, String field) {
        if (name == null || !Pattern.matches("[А-Яа-яA-Za-z]+", name))
            throw new ValidationException(field + " должно содержать только буквы");
    }
    ...
    public void setLogin(String login) { validateLogin(login); this.login =
            login; }
    public void setPassword(String password, String confirmPassword) {
        validatePassword(password, confirmPassword); this.password = password;
        this.confirmPassword = confirmPassword; }
    public void setLastName(String lastName) { validateName(lastName,
            "Фамилия"); this.lastName = lastName; }
    public void setFirstName(String firstName) { validateName(firstName,
            "Имя"); this.firstName = firstName; }
    public void setPatronymic(String patronymic) { if (patronymic != null)
        validateName(patronymic, "Отчество"); this.patronymic = patronymic; }
    public void setAge(Integer age) { if (age != null && age < 0) throw new
            ValidationException("Возраст не может быть отрицательным"); this.age = age; }
